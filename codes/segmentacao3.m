function [m_aux,m] = segmentacao3(m_aux,m,p,u)
% u = 0.1;
% p = 60;
if rem(p,2) ~= 0
    p = p + 1;
...
%     ch_cont = 0;
    for k = 1:Ns
        ...
        if bR == aR
            ...
            input.RRI.RRI_segL1(cont_k,k) = aL;
        ...
        if cont_k >= p
            ...
            if((length(find((input.RRI.RRI_segL1(:,k)) == 2))) >= round(u*p))
                input.RRI.RRI_segL(:,k) = [0;0;1];
                input.RRI.RRI_segLD(1,k) = 2;
            elseif((length(find((input.RRI.RRI_segL1(:,k)) == 0))) >= round(0.5)) % 0.5?
        ...
        if (((j+1) <= (N-1)) && ((k+1) <= Ns))
                ...
                cont_k = p - p2;
%                 ch_cont = (2*cont_k);
                input.RRI.RRI_segIDX(1:(2*cont_k),k) = input.RRI.RRI_segIDX((2*p2+1):2*p2,k-1);
                input.RRI.RRI_segP(1:cont_k,k) = input.RRI.RRI_segP((p2+1):p,k-1);
                input.RRI.RRI_segA(1:cont_k,k) = input.RRI.RRI_segA((p2+1):p,k-1);
                input.RRI.RRI_segL1(1:cont_k,k) = input.RRI.RRI_segL1((p2+1):p,k-1);
%                 input.RRI.RRI_segEBC(1:(cont_k*E),k) = input.RRI.RRI_segEBC((p2*E + 1):end,k-1);
                input.RRI.RRI_segWT(1:cont_k,k) = input.RRI.RRI_segWT((p2+1):p,k-1);
                input.RRI.RRI_segWU(1:cont_k,k) = input.RRI.RRI_segWU((p2+1):p,k-1);
                input.RRI.RRI_segWP(1:cont_k,k) = input.RRI.RRI_segWP((p2+1):p,k-1);
            end
        end
        j = j + 1;
    end
